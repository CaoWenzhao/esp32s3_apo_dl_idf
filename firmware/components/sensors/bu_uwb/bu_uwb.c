...
static char s_line_buffer[BU_UWB_LINE_MAX];
static size_t s_line_position;
...
esp_err_t bu_uwb_read_line(char *line, size_t line_size, uint32_t timeout_ms)
{
    ...
    while (xTaskGetTickCount() - start < timeout) {
        ...
        if (character == '\n') {
            if (s_line_position == 0) continue;
            if (s_line_position + 1 > line_size) {
                s_line_position = 0;
                s_line_buffer[0] = '\0';
                return ESP_ERR_INVALID_SIZE;
            }
            memcpy(line, s_line_buffer, s_line_position);
            line[s_line_position] = '\0';
            s_line_position = 0;
            s_line_buffer[0] = '\0';
            return ESP_OK;
        }
        if (s_line_position + 1 >= sizeof(s_line_buffer)) {
            s_line_position = 0;
            s_line_buffer[0] = '\0';
            return ESP_ERR_INVALID_SIZE;
        }
        s_line_buffer[s_line_position++] = (char)character;
        s_line_buffer[s_line_position] = '\0';
    }
